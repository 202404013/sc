...
def home(request):
	...
	if request.method == 'POST':
		...
		if user is not None:
			login(request, user)
			messages.success(request, "Welcome, fellow Carpenters!")
...
def register_user(request):
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			login(request, user)
			messages.success(request, "Welcome! You will now be working late as a fellow Carpenters.")
			return redirect('home')
	else:
		form = SignUpForm()
...
def customer_record(request, pk):
	if request.user.is_authenticated:
		...
		messages.success(request, "Oops! You must be logged in to view the page requested.")
		return redirect('home')
